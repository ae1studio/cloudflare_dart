// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'email_rules.dart';

// **************************************************************************
// BuiltValueGenerator
// **************************************************************************

class _$EmailRules extends EmailRules {
  @override
  final BuiltList<EmailRuleAction>? actions;
  @override
  final bool? enabled;
  @override
  final String? id;
  @override
  final BuiltList<EmailRuleMatcher>? matchers;
  @override
  final String? name;
  @override
  final num? priority;
  @override
  final String? tag;

  factory _$EmailRules([void Function(EmailRulesBuilder)? updates]) =>
      (EmailRulesBuilder()..update(updates))._build();

  _$EmailRules._(
      {this.actions,
      this.enabled,
      this.id,
      this.matchers,
      this.name,
      this.priority,
      this.tag})
      : super._();
  @override
  EmailRules rebuild(void Function(EmailRulesBuilder) updates) =>
      (toBuilder()..update(updates)).build();

  @override
  EmailRulesBuilder toBuilder() => EmailRulesBuilder()..replace(this);

  @override
  bool operator ==(Object other) {
    if (identical(other, this)) return true;
    return other is EmailRules &&
        actions == other.actions &&
        enabled == other.enabled &&
        id == other.id &&
        matchers == other.matchers &&
        name == other.name &&
        priority == other.priority &&
        tag == other.tag;
  }

  @override
  int get hashCode {
    var _$hash = 0;
    _$hash = $jc(_$hash, actions.hashCode);
    _$hash = $jc(_$hash, enabled.hashCode);
    _$hash = $jc(_$hash, id.hashCode);
    _$hash = $jc(_$hash, matchers.hashCode);
    _$hash = $jc(_$hash, name.hashCode);
    _$hash = $jc(_$hash, priority.hashCode);
    _$hash = $jc(_$hash, tag.hashCode);
    _$hash = $jf(_$hash);
    return _$hash;
  }

  @override
  String toString() {
    return (newBuiltValueToStringHelper(r'EmailRules')
          ..add('actions', actions)
          ..add('enabled', enabled)
          ..add('id', id)
          ..add('matchers', matchers)
          ..add('name', name)
          ..add('priority', priority)
          ..add('tag', tag))
        .toString();
  }
}

class EmailRulesBuilder
    implements
        Builder<EmailRules, EmailRulesBuilder>,
        EmailRulePropertiesBuilder {
  _$EmailRules? _$v;

  ListBuilder<EmailRuleAction>? _actions;
  ListBuilder<EmailRuleAction> get actions =>
      _$this._actions ??= ListBuilder<EmailRuleAction>();
  set actions(covariant ListBuilder<EmailRuleAction>? actions) =>
      _$this._actions = actions;

  bool? _enabled;
  bool? get enabled => _$this._enabled;
  set enabled(covariant bool? enabled) => _$this._enabled = enabled;

  String? _id;
  String? get id => _$this._id;
  set id(covariant String? id) => _$this._id = id;

  ListBuilder<EmailRuleMatcher>? _matchers;
  ListBuilder<EmailRuleMatcher> get matchers =>
      _$this._matchers ??= ListBuilder<EmailRuleMatcher>();
  set matchers(covariant ListBuilder<EmailRuleMatcher>? matchers) =>
      _$this._matchers = matchers;

  String? _name;
  String? get name => _$this._name;
  set name(covariant String? name) => _$this._name = name;

  num? _priority;
  num? get priority => _$this._priority;
  set priority(covariant num? priority) => _$this._priority = priority;

  String? _tag;
  String? get tag => _$this._tag;
  set tag(covariant String? tag) => _$this._tag = tag;

  EmailRulesBuilder() {
    EmailRules._defaults(this);
  }

  EmailRulesBuilder get _$this {
    final $v = _$v;
    if ($v != null) {
      _actions = $v.actions?.toBuilder();
      _enabled = $v.enabled;
      _id = $v.id;
      _matchers = $v.matchers?.toBuilder();
      _name = $v.name;
      _priority = $v.priority;
      _tag = $v.tag;
      _$v = null;
    }
    return this;
  }

  @override
  void replace(covariant EmailRules other) {
    _$v = other as _$EmailRules;
  }

  @override
  void update(void Function(EmailRulesBuilder)? updates) {
    if (updates != null) updates(this);
  }

  @override
  EmailRules build() => _build();

  _$EmailRules _build() {
    _$EmailRules _$result;
    try {
      _$result = _$v ??
          _$EmailRules._(
            actions: _actions?.build(),
            enabled: enabled,
            id: id,
            matchers: _matchers?.build(),
            name: name,
            priority: priority,
            tag: tag,
          );
    } catch (_) {
      late String _$failedField;
      try {
        _$failedField = 'actions';
        _actions?.build();

        _$failedField = 'matchers';
        _matchers?.build();
      } catch (e) {
        throw BuiltValueNestedFieldError(
            r'EmailRules', _$failedField, e.toString());
      }
      rethrow;
    }
    replace(_$result);
    return _$result;
  }
}

// ignore_for_file: deprecated_member_use_from_same_package,type=lint
