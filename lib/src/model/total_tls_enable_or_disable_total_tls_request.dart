//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_element
import 'package:cloudflare_dart/src/model/tls_certificates_and_hostnames_components_schemas_certificate_authority.dart';
import 'package:built_value/built_value.dart';
import 'package:built_value/serializer.dart';

part 'total_tls_enable_or_disable_total_tls_request.g.dart';

/// TotalTlsEnableOrDisableTotalTlsRequest
///
/// Properties:
/// * [enabled] - If enabled, Total TLS will order a hostname specific TLS certificate for any proxied A, AAAA, or CNAME record in your zone.
/// * [certificateAuthority] 
@BuiltValue()
abstract class TotalTlsEnableOrDisableTotalTlsRequest implements Built<TotalTlsEnableOrDisableTotalTlsRequest, TotalTlsEnableOrDisableTotalTlsRequestBuilder> {
  /// If enabled, Total TLS will order a hostname specific TLS certificate for any proxied A, AAAA, or CNAME record in your zone.
  @BuiltValueField(wireName: r'enabled')
  bool get enabled;

  @BuiltValueField(wireName: r'certificate_authority')
  TlsCertificatesAndHostnamesComponentsSchemasCertificateAuthority? get certificateAuthority;
  // enum certificateAuthorityEnum {  google,  lets_encrypt,  ssl_com,  };

  TotalTlsEnableOrDisableTotalTlsRequest._();

  factory TotalTlsEnableOrDisableTotalTlsRequest([void updates(TotalTlsEnableOrDisableTotalTlsRequestBuilder b)]) = _$TotalTlsEnableOrDisableTotalTlsRequest;

  @BuiltValueHook(initializeBuilder: true)
  static void _defaults(TotalTlsEnableOrDisableTotalTlsRequestBuilder b) => b;

  @BuiltValueSerializer(custom: true)
  static Serializer<TotalTlsEnableOrDisableTotalTlsRequest> get serializer => _$TotalTlsEnableOrDisableTotalTlsRequestSerializer();
}

class _$TotalTlsEnableOrDisableTotalTlsRequestSerializer implements PrimitiveSerializer<TotalTlsEnableOrDisableTotalTlsRequest> {
  @override
  final Iterable<Type> types = const [TotalTlsEnableOrDisableTotalTlsRequest, _$TotalTlsEnableOrDisableTotalTlsRequest];

  @override
  final String wireName = r'TotalTlsEnableOrDisableTotalTlsRequest';

  Iterable<Object?> _serializeProperties(
    Serializers serializers,
    TotalTlsEnableOrDisableTotalTlsRequest object, {
    FullType specifiedType = FullType.unspecified,
  }) sync* {
    yield r'enabled';
    yield serializers.serialize(
      object.enabled,
      specifiedType: const FullType(bool),
    );
    if (object.certificateAuthority != null) {
      yield r'certificate_authority';
      yield serializers.serialize(
        object.certificateAuthority,
        specifiedType: const FullType(TlsCertificatesAndHostnamesComponentsSchemasCertificateAuthority),
      );
    }
  }

  @override
  Object serialize(
    Serializers serializers,
    TotalTlsEnableOrDisableTotalTlsRequest object, {
    FullType specifiedType = FullType.unspecified,
  }) {
    return _serializeProperties(serializers, object, specifiedType: specifiedType).toList();
  }

  void _deserializeProperties(
    Serializers serializers,
    Object serialized, {
    FullType specifiedType = FullType.unspecified,
    required List<Object?> serializedList,
    required TotalTlsEnableOrDisableTotalTlsRequestBuilder result,
    required List<Object?> unhandled,
  }) {
    for (var i = 0; i < serializedList.length; i += 2) {
      final key = serializedList[i] as String;
      final value = serializedList[i + 1];
      switch (key) {
        case r'enabled':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(bool),
          ) as bool;
          result.enabled = valueDes;
          break;
        case r'certificate_authority':
          final valueDes = serializers.deserialize(
            value,
            specifiedType: const FullType(TlsCertificatesAndHostnamesComponentsSchemasCertificateAuthority),
          ) as TlsCertificatesAndHostnamesComponentsSchemasCertificateAuthority;
          result.certificateAuthority = valueDes;
          break;
        default:
          unhandled.add(key);
          unhandled.add(value);
          break;
      }
    }
  }

  @override
  TotalTlsEnableOrDisableTotalTlsRequest deserialize(
    Serializers serializers,
    Object serialized, {
    FullType specifiedType = FullType.unspecified,
  }) {
    final result = TotalTlsEnableOrDisableTotalTlsRequestBuilder();
    final serializedList = (serialized as Iterable<Object?>).toList();
    final unhandled = <Object?>[];
    _deserializeProperties(
      serializers,
      serialized,
      specifiedType: specifiedType,
      serializedList: serializedList,
      unhandled: unhandled,
      result: result,
    );
    return result.build();
  }
}

